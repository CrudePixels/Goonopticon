...
function LogDev(Message, Type = "miscellaneous", Cb)
{
    ...
    */
    ...
    if (browser_polyfill && browser_polyfill.runtime && browser_polyfill.runtime.sendMessage) {
        const entry = {
        ...
        browser_polyfill.runtime.sendMessage({ type: "devlog", entry })
            .then((response) => {
...
browser_polyfill.runtime.onMessage.addListener((msg, sender, sendResponse) =>
{
    ...
    {
        browser_polyfill.storage.local.get(["PodAwful::DevLog"]).then(result => {
            ...
            browser_polyfill.storage.local.set({ "PodAwful::DevLog": DevLog }).then(() => {
...
browser_polyfill.runtime.onInstalled.addListener((details) =>
{
...
async function checkForUpdates(forceCheck = false) {
    try {
        ...
        // Get current version
        const currentVersion = browser_polyfill.runtime.getManifest().version;
        ...
        if (!forceCheck) {
            const lastCheck = await browser_polyfill.storage.local.get(['lastUpdateCheck']);
            const now = Date.now();
        ...
        const now = Date.now();
        await browser_polyfill.storage.local.set({ lastUpdateCheck: now });
        ...
        if (isNewerVersion(latestVersion, currentVersion)) {
            ...
            // Show notification
            browser_polyfill.notifications.create({
                type: 'basic',
            ...
            await browser_polyfill.storage.local.set({ updateAvailable: false });
        }
